...
def main():
    ...
    all_sums.sort()
    remaining_free_space = MAX_STORAGE - all_sums[-1]
    min_directory_to_delete = REQUIRED_SPACE - remaining_free_space
    
    for size in all_sums:
        if size >= min_directory_to_delete:
            print(size)
            break
